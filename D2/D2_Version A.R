...
getwd()
mydata1 <- readRDS("growth_data.rds")
str(mydata1)
...
str(mydata1)
summary(mydata1)
with(mydata1, tapply(final_g, cond, length))
...
library(ggplot2)
ggplot(mydata1, aes(y=final_g, x=inc))+geom_point()+ facet_grid(~cond)
...
library(nlme)
mydata1.lm=lme(final_g~inc-1,random=~1|cond, data=mydata1)
...
mydata1 <- readRDS("stress_data.rds")
str(mydata1)
summary(mydata1) #balanced design
with(mydata1, tapply(output, vacuum, mean))


#2)we can see in the plot that  between refrigeration and vacuum is a positive interaction visible, but needs to be proven.
library(ggplot2)
ggplot(mydata1, aes(y=stress, x=mix,col=temp)) + geom_point()

#3)
mydata1.lm = lm(stress~mix*temp, data=mydata1)
summary(mydata1.lm)#overall f test,  for p-value: 0.01165, significant! 
library(car)
Anova(mydata1.lm, type=2) #partial F test
...
df = data.frame(cond="C2", data=mydata1)
df$pred = predict(mydata1.lm, df, level=0)
df
