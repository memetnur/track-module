...
h=ggplot(data=iris, aes(x=Sepal.Length, y=Sepal.Width, shape=Species)) +geom_point()
h
# Exercise
...
ggplot(iris, aes(Species, Sepal.Width))+geom_boxplot()+geom_point(position="jitter")
...
ggplot(iris, aes(x = Petal.Length, y = Sepal.Width, colour = Species)) + geom_point() + geom_smooth()
...
p1 = base + facet_grid(Species ~ .)
p2 = base + facet_grid(. ~ Species)
